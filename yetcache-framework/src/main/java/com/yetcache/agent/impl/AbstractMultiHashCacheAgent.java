package com.yetcache.agent.impl;
...
import com.yetcache.utils.LockUtils;
import com.yetcache.utils.RedisSafeUtils;
import com.yetcache.agent.IMultiHashCacheAgent;
import com.yetcache.enums.CacheType;
import com.yetcache.enums.MultiHashCacheEventType;
import com.yetcache.event.CacheEvent;
import com.yetcache.event.MultiHashCacheEvent;
import com.yetcache.source.IMultiHashCacheSourceService;
import com.yetcache.utils.CacheRetryUtils;
...
public abstract class AbstractMultiHashCacheAgent<E> extends AbstractMultiKeyCacheAgent<E>
    ...
    protected void updateCache(E data) {
        if (isTenantScoped()) {
    ...
    protected void preventCachePenetration(String key) {
        if (Boolean.TRUE.equals(redisTemplate.hasKey(key))) {
            return; // 已存在缓存，不执行穿透防护
        }

        Map<String, E> emptyPlaceholderEntries = new HashMap<>();
        emptyPlaceholderEntries.put(EMPTY_OBJ_HASH_KEY, createEmptyPlaceholder());

        RedisSafeUtils.safeReplaceHash(key, emptyPlaceholderEntries, hashOperations, redisTemplate);
        redisTemplate.expire(key, properties.getPenetrationProtectTtlSecs(), TimeUnit.SECONDS);
        log.debug("[穿透保护] key={} 写入空值，占位TTL={}秒", key, properties.getPenetrationProtectTtlSecs());
    }
