 ...
 * @since 2025/7/23
 */
public class DefaultRabbitmqCacheBroadcastSenderBak implements CacheBroadcastSenderBak {
    private final RabbitMqConfig config;
    private final RabbitTemplate rabbitTemplate;
    private final MessageConverter messageConverter;

    public DefaultRabbitmqCacheBroadcastSenderBak(RabbitMqConfig config, RabbitTemplate rabbitTemplate) {
        this.config = config;
    ...
    @Override
    public void send(AbstractCacheBroadcastCommand command) {
        try {
